...
def searchproduct(request):
    ...
    worddata=word.split(" ")
    for i in worddata:
        a=Product.objects.filter(Q(categoryname__categoryname__icontains=i)|Q(name__icontains=i)|Q(price__icontains=i)).distinct()
    return render(request,'product.html',{'data':a})
...
def orderdetails(request,id):
    if 'email' in request.session:
        ...
        cartdata=Cart.objects.filter(orderid=orderdata.pk)
        prolist=[]
        for i in cartdata:
            prodict={}
            productdata=Product.objects.get(id=i.productid)
            prodict['img']=productdata.image
            prodict['name']=productdata.name
            prodict['quantity']=i.quantity
            prodict['total']=i.totalprice
            prolist.append(prodict)
        return render(request,'orderdetails.html',{'orderdata':orderdata,'pro':prolist})
    else:
        return redirect('login')